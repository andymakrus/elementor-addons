...
class Dici_Feature_Pack_Elementor_HF{
	...
	public function enqueue_scripts() {
		...
		if ( class_exists( '\Elementor\Post_CSS_File' ) ) {
			...
			if ( self::header_enabled() ) {
				$css_file = new \Elementor\Post_CSS_File( self::get_header_id() );
				$css_file->enqueue();
			}

			if ( self::footer_enabled() ) {
				$css_file = new \Elementor\Post_CSS_File( self::get_footer_id() );
				$css_file->enqueue();
			}
		}
	}
	...
	public function elementor_not_available() {
		...
		echo '<p>' . sprintf( __( 'The <strong>Header Footer Elementor</strong> plugin requires <strong><a href="%s">Elementor</strong></a> plugin installed & activated.', 'dici-feature-pack' ) . '</p>', $url );
		echo '</div>';
	}
	...
	public function body_class( $classes ) {
		...
		if ( self::header_enabled() ) {
			$classes[] = 'dici-hf-header';
		}

		if ( self::footer_enabled() ) {
			$classes[] = 'dici-hf-footer';
		}

		$classes[] = 'dici-hf-template-' . $this->template;
		$classes[] = 'dici-hf-stylesheet-' . get_stylesheet();
	...
	public static function get_settings( $setting = '', $default = '' ) {
		if ( 'type_header' == $setting || 'type_footer' == $setting ) {
			...
			$template = apply_filters( "dici_hf_get_settings_{$setting}", $template );
	...
	public static function header_enabled() {
		...
		return apply_filters( 'dici_hf_header_enabled', $status );
	}
	...
	public static function footer_enabled() {
		...
		return apply_filters( 'dici_hf_footer_enabled', $status );
	}
	...
	public static function get_header_id() {
		...
		return apply_filters( 'get_dici_hf_header_id', $header_id );
	}
	...
	public static function get_footer_id() {
		...
		return apply_filters( 'get_dici_hf_footer_id', $footer_id );
	}
	...
	public static function header_wrapper_start(){
	    ?>
        <header id="masthead" itemscope="itemscope" itemtype="https://schema.org/WPHeader" class="<?php echo esc_attr__( ( defined( 'TZ_THEME_PREFIX' ) ? TZ_THEME_PREFIX.'-elementor-header' : 'dici-elementor-header' ) )?>">
	...
	public static function render_header() {
		...
		if ( false == apply_filters( 'enable_dici_hf_render_header', true ) ) {
			return;
		}
	...
	public static function footer_wrapper_start(){
        ?>
        <aside itemtype="https://schema.org/WPFooter" itemscope="itemscope" id="colophon" role="contentinfo" class="<?php echo esc_attr__( ( defined( 'THEME_PREFIX' ) ? THEME_PREFIX.'-elementor-footer' : 'dici-elementor-footer' ) )?>">
        <?php
    }
	...
	public static function render_footer() {
		...
		if ( false == apply_filters( 'enable_dici_hf_render_footer', true ) ) {
			return;
		}
